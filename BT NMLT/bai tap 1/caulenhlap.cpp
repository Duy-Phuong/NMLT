...
//ham tinh so luong uoc so cua n
int demuoc(int n)
{
	int dem = 0;
	for (int i = 1; i < n; i++)
...
//ham tinh trung binh cong uoc so chan
int trungbinhcong(int n)
{
	int dem = 0;
	float s = 0;
	for (int i = 1; i < n; i++)
...
//ham tim uoc so le lon nhat
void timuoclemax(int n)
{
	int max = 1;
	for (int i = 1; i <= n; i++)
	{
...
void main()
{
	...
	printf("gia tri cua s2 la: %d\n", s2);
}

//ham kiem tra so co dang 2^k
bool kiemtra(int n)
{
	while (n % 2 == 0)
	{
...
//in tam giac co dau sao o trong co chieu cao h
void InTamGiacDac(int h)
{
	for (int i = 1; i <= h; i++)
	{
		for (int j = 1; j <= i; j++)
